...
export default function Offer() {
  ...
  return (
    <div className="pt-8 pb-24 min-h-screen relative">
      <div className="container mx-auto px-6 max-w-6xl relative z-10">

        <div className="text-center mb-16 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div className="w-16 h-16 bg-cyan-100 text-cyan-600 rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-inner rotate-3">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 relative z-20">
          {courses.map((course, idx) => (
            <div
              key={idx}
              className={`flex flex-col relative overflow-hidden bg-white/70 backdrop-blur-xl border border-white p-8 rounded-[2rem] shadow-xl shadow-slate-200/50 hover:-translate-y-2 transition-transform duration-300 animate-in fade-in slide-in-from-bottom-8`}
              style={{ animationDelay: `${(idx + 1) * 150}ms`, animationFillMode: 'both' }}
            >

              {/* Absolutna wstążka / Badge */}
              {course.badge && (
                <div className="absolute top-6 -right-10 bg-gradient-to-r from-indigo-500 to-cyan-500 text-white py-1 px-10 rotate-45 text-xs font-bold uppercase tracking-wider shadow-md">
                  {course.badge}
                </div>
              )}

              <div className="mb-6 pr-8">
                <h3 className="text-2xl font-bold font-['Outfit'] text-slate-800 mb-3">{course.title}</h3>
              ...
              <div className="mb-8 p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                <p className="font-semibold text-sm text-slate-800 mb-3">Co w pakiecie?</p>
              ...
              <div className="mt-auto flex items-center justify-between border-t border-slate-100 pt-6">
                <span className="font-bold text-lg text-indigo-700">{course.price}</span>
                {/* 
                  Aby podmienić pod platformę wystarczy zamienić # na odpowiedni link do konkretnego kursu. 
                */}
                <a href="#" className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-full font-medium transition-colors text-sm">
      ...
      {/* Dekoracyjne Tło Kart */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-gradient-to-tr from-indigo-50/50 to-cyan-50/50 rounded-full blur-3xl -z-10 pointer-events-none"></div>
    </div>
  );
}
